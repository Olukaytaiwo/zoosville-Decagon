...
const formatAnimal = animal => {
  ...
  return `
  <div id = 'animaList' class='animal' data-animal='${JSON.stringify(animal)}'>
      <div> <h3>     </h3>    <h3 id = "nom"> Name: <strong> ${name} <strong> </h3> </div>
      <div id = "speci"> Specie: <strong> ${specie} </strong></div>
      <div> <h4 id = "clas"> Class: <strong> ${classes} </strong></h4> </div>
      <div> <h5 id = "inf"> Information:  ${info}  </h5>  </div>
  </div>
  <hr>
  `;
};
